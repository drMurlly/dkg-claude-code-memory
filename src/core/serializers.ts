...
export function serializeToQuads(artifact: ArtifactRecord): RdfQuad[] {
  ...
  const quads: RdfQuad[] = [
    ...
    // Sub-agent provenance fields
    { subject: provenanceId, predicate: `${WM}subAgentId`, object: lit(artifact.provenance.subAgentId ?? '') },
    { subject: provenanceId, predicate: `${WM}parentTaskId`, object: lit(artifact.provenance.parentTaskId ?? '') },
    { subject: provenanceId, predicate: `${WM}agentRole`, object: lit(artifact.provenance.agentRole ?? '') },
